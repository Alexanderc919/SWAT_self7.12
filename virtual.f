...
!! sum HRU results for subbasin 
      ...
      !! subbasin averages: water
        ...
        snom_sub(sb,1) =snom_sub(sb,1) + snom_hru(1)* hru_fr(j)!½µÑ©
        snom_sub(sb,2) =snom_sub(sb,2) + snom_hru(2)* hru_fr(j)
        snom_sub(sb,3) =snom_sub(sb,3) + snom_hru(3)*hru_fr(j)
        !print*,snom_sub(sb,1),snom_sub(sb,2),snom_sub(sb,3)
          if (snom_sub(sb,3)>0)print*,snom_sub(sb,3)
...
!        sub_minps(sb) = sub_minps(sb) / sub_fr(sb)
     ...
     &                              ratio = sub_hwyld(ii) / sub_wyld(sb)
        ...
        if (curyr > nyskip) then
         ...
         !ÔÂ³ß¶È
         snom_sub1(sb,1)= snom_sub1(sb,1)+snom_sub(sb,1)
         snom_sub1(sb,2)= snom_sub1(sb,2)+snom_sub(sb,2)
         snom_sub1(sb,3)= snom_sub1(sb,3)+snom_sub(sb,3)
