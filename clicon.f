...
!!  tmaband£¨(:,:)|deg C         |accumulated temperature for the day in band in HRU
...
!! Elevation Adjustments !!
     ...
     &                                       * plaps(hru_sub(k)) / 1000.
          ...
          !! ¼ÆËã»ýÎÂ
          if(tmnband(ib,k)>0.)then
          tmaband(ib,k)=2*tmxband(ib,k)+(3.141592-2)* tmnband(ib,k)
          else if(tmnband(ib,k)>0..and. tmnband(ib,k)<0.)then
               tmaband(ib,k)=2*( tmxband(ib,k) - tmnband(ib,k))
     &         *cos(asin(tmnband(ib,k)/(tmnband(ib,k)-tmxband(ib,k))))
     & +(3.14159-2*asin(tmnband(ib,k)/(tmnband(ib,k)-tmxband(ib,k))))
     &          *tmnband(ib,k)
          else
              tmaband(ib,k)=0.
          end if
