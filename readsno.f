...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do
        ...
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_crain(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_sftmp_accu(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_smtmp(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_smtmp_accu(ib,i), ib = 1, 10)
        if (eof < 0) exit
        ...
        read (113,1000,iostat=eof) (sub_timp(ib,i), ib = 1, 10)
         if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_Elesnoline(ib,i), ib = 1, 10)
        
        
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_CT(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_icefmx(ib,i), ib = 1, 10)
        if (eof < 0) exit
        read (113,1000,iostat=eof) (sub_icefmn(ib,i), ib = 1, 10)
        if (eof < 0) exit
        exit 
      end do
      ...
      do ib = 1, 10
          ...
          if (sub_crain(ib,i) <= 0.) sub_crain(ib,i) = crain
	if (sub_sftmp_accu(ib,i) <= 0.) sub_sftmp_accu(ib,i) = sftmp_accu
          if (sub_smtmp(ib,i) <= 0.) sub_smtmp(ib,i) = smtmp
      if (sub_smtmp_accu(ib,i) <= 0.) sub_smtmp_accu(ib,i) = smtmp_accu
          if (sub_smfmx(ib,i) <= 0.) sub_smfmx(ib,i) = smfmx
          if (sub_smfmn(ib,i) <= 0.) sub_smfmn(ib,i) = smfmn
          if (sub_timp(ib,i) <= 0.) sub_timp(ib,i) = timp
          
      if (sub_Elesnoline(ib,i) <= 0.) sub_Elesnoline(ib,i) = Elesnoline
          if (sub_CT(ib,i) <= 0.) sub_CT(ib,i) = CT
          if (sub_icefmx(ib,i) <= 0.) sub_icefmx(ib,i) = icefmx
          if (sub_icefmn(ib,i) <= 0.) sub_icefmn(ib,i) = icefmn
      end do
