...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &  sub_smfmn(1,i), sub_sftmp(1,i),sub_crain(1,i),
     &  sub_sftmp_accu(1,i),sub_smtmp(1,i), sub_smtmp_accu(1,i),
     &  sub_timp(1,i),sub_Elesnoline(1,i),sub_CT(1,i),sub_icefmx(1,i),
     &  sub_icefmn(1,i)
        do ib = 2, 10
          sub_smfmx(ib,i) = sub_smfmx(1,i)
          sub_smfmn(ib,i) = sub_smfmn(1,i)
          sub_sftmp(ib,i) = sub_sftmp(1,i)
          sub_crain(1,i) = sub_crain(1,i)
         ! sub_sftmp_max(ib,i) = sub_sftmp_max(1,i)
          sub_sftmp_accu(ib,i) = sub_sftmp_accu(1,i)
          sub_smtmp(ib,i) = sub_smtmp(1,i) 
          sub_smtmp_accu(ib,i) = sub_smtmp_accu(1,i) 
          sub_timp(ib,i) = sub_timp(1,i)
          sub_Elesnoline(ib,i) = sub_Elesnoline(1,i)
          sub_CT(ib,i) = sub_CT(1,i)
          sub_icefmx(ib,i) = sub_icefmx(1,i)
          sub_icefmn(ib,i) = sub_icefmn(1,i)
        end do
      else
        read (101,5100) titldum                                      
      end if
...
!!    set default values
	 ...
	 if (sub_smtmp_accu(ib,i) < 1.e-6) sub_smtmp_accu(ib,i) = smtmp_accu  
        if (sub_sftmp(ib,i) < 1.e-6) sub_sftmp(ib,i) = sftmp 
      if (sub_crain(ib,i) < 1.e-6) sub_crain(ib,i) = crain
        !!if(sub_crain(ib,i)< 1.e-6)) sub_crain(ib,i) = crain
       !! if (sub_sftmp_max(ib,i) < 1.e-6) sub_sftmp_max(ib,i) = sftmp_max  
	 if (sub_sftmp_accu(ib,i) < 1.e-6) sub_sftmp_accu(ib,i) = sftmp_accu   
      ...
      if (sub_Elesnoline(ib,i)< 1.e-6) sub_Elesnoline(ib,i) = Elesnoline
        if (sub_CT(ib,i) < 1.e-6) sub_CT(ib,i) = CT
        if (sub_icefmx(ib,i) < 1.e-6) sub_icefmx(ib,i) = icefmx
        if (sub_icefmn(ib,i) < 1.e-6) sub_icefmn(ib,i) = icefmn
      end do
